export default async function GetInvestmentPriceHandler(req, res) {
	try {
		...
		const stock_res = await fetch(
			`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${body.ticker}&apikey=${process.env.AV_API_KEY}`
		);
		...
		if (
			...
			stock_data["Global Quote"]["05. price"]
		) {
